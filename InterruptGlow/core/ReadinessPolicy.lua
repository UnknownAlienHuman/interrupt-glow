...
local function ReadPetActionUsability(slot)
    if type(GetPetActionSlotUsable) ~= "function" or type(slot) ~= "number" then
        return nil, true
    end

    -- Blizzard PetActionBar uses this API separately from GetPetActionInfo.
    -- GetPetActionInfo's eighth and ninth returns are checksRange/inRange, not
    -- usability. Treat any inaccessible/non-boolean result as hard restricted.
    local ok, isUsable = pcall(GetPetActionSlotUsable, slot)
...
local function ApplyPetGate(ability)
    ...
    local usable, restricted = ReadPetActionUsability(ability.sourceID)
    if restricted then
...
IG:RegisterModule("ReadinessPolicy", {
    ...
    petUsabilitySource = "GetPetActionSlotUsable",
